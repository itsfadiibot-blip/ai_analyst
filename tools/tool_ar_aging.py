# -*- coding: utf-8 -*-
"""Tool: get_ar_aging — Accounts Receivable aging summary with aging buckets."""
import logging
...
@register_tool
class ARAgingTool(BaseTool):
    name = 'get_ar_aging'
    description = (
        'Accounts Receivable aging summary. Groups outstanding customer invoices '
        'into aging buckets: Current (not yet due), 1-30 days overdue, 31-60, 61-90, '
        'and 90+ days overdue. Shows amounts by customer.'
    )
    parameters_schema = {
        'type': 'object',
        'properties': {
    ...
    def execute(self, env, user, params):
        ...
        # Find open (not fully reconciled) customer invoices
        domain = [
        ...
        open_invoices = AccountMove.search_read(
            domain,
            fields=[
            ...
            limit=500,  # Read more, then group
        )
        ...
        for inv in open_invoices:
            ...
            amount = inv.get('amount_residual', 0) or 0
            grand_total += amount
            ...
            partner_aging[partner_id][bucket_key] += amount
            partner_aging[partner_id]['total'] += amount

        # Sort partners by total outstanding and limit
        partner_rows = sorted(
            partner_aging.values(),
        ...
        return {
            ...
            'buckets': buckets,
            'by_partner': partner_rows,
            'grand_total': round(grand_total, 2),
            'total_partners': len(partner_rows),
            'currency': currency,
        }
